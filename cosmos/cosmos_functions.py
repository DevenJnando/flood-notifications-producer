...
from connections import cosmosdb_client
import app.main
from cosmos.cosmos_queries import area_query, districts_query


def get_shard_keys() -> list[str] | None:
    database_names: list[str] = []
    container = cosmosdb_client.get_shardmap_container(app.main.postcodes_cosmos_client)
...
async def match_area_to_flood_geometry(flood_geometry: dict[str, Any], database_name: str) -> ItemPaged[dict] | None:
    ...
    container = cosmosdb_client.get_postcodes_area_container(app.main.postcodes_cosmos_client, database_name)
    if container is None:
        return None
    try:
...
async def match_districts_to_area(area: dict[str, Any]) -> ItemPaged[dict] | None:
    ...
    container = cosmosdb_client.get_postcodes_district_container(app.main.postcodes_cosmos_client, area["areaCode"])
    if container is None:
        return None
    try:
...
async def match_district_to_geometry(flood_geometry: dict[str, Any], district: str) -> ItemPaged[dict] | None:
    ...
    container = cosmosdb_client.get_postcodes_district_container(app.main.postcodes_cosmos_client, area_code)
    if container is None:
        return None
    try:
...
async def match_full_postcode_to_geometry(flood_geometry: dict[str, Any], district: str) -> ItemPaged[dict] | None:
    parameters = [dict(name="@geometry", value=flood_geometry)]
    area_code = re.split(r'(^[^\d]+)', district)[1:][0]
    container = cosmosdb_client.get_full_postcodes_container(app.main.postcodes_cosmos_client, area_code)
    if container is None:
        return None
    try:
        return container.query_items(query=districts_query(), parameters=parameters, partition_key=district)
