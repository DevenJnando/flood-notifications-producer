...
async def match_area_to_districts(areas: list[dict[str, Any]]):
    matches = [match_districts_to_area(area) for area in areas]
    return await asyncio.gather(*matches)


async def match_districts_to_flood_geometry(flood_geometry: dict[str, Any], districts: list[str]):
    matches = [match_district_to_geometry(flood_geometry, district) for district in districts]
    return await asyncio.gather(*matches)


async def match_full_postcodes_to_flood_geometry(flood_geometry: dict[str, Any], districts: list[str]):
    matches = [match_full_postcode_to_geometry(flood_geometry, district) for district in districts]
    return await asyncio.gather(*matches)


async def areas_in_flood_range(flood_geometry: dict[str, Any], database_names: list[str]):
...
async def districts_in_area(areas: list[dict[str, Any]]):
...
async def full_postcodes_in_flood_range(flood_geometry: dict[str, Any], districts: list[str]):
